    ...
    inner class CreateConnectThread(bluetoothAdapter: BluetoothAdapter, address: String) :
        ...
        init {
            val bluetoothDevice: BluetoothDevice =
                bluetoothAdapter.bondedDevices.first { it.name == address }
                    ...
                    if (sendPacket.isNotEmpty()) {
                        sendPacket.poll()?.let {
                            ...
                            val escaped = PacketUtils.EscTxPacket(packet).run {
                                val checksum =
                                    PacketUtils.calculateChecksum(this.substring(2, this.length))
                                this + checksum[1].toInt().toChar() + checksum[0].toInt()
                                    .toChar() + END_PACKET_SYMBOL
                            }
                    ...
                    if (reader.ready()) {
                        ...
                        reader.read().takeIf { it != -1 }?.toChar()?.let {
                            ...
                            if (mSecu3PacketSearch == SECU3_PACKET_SEARCH.SEARCH_END && it == '\r') {
                                mSecu3PacketSearch = SECU3_PACKET_SEARCH.SEARCH_START
                                val escaped = PacketUtils.EscRxPacket(
                                    packetBuffer.sliceArray(
                                        IntRange(
                                            0,
                                            idx - 4
                                        )
                                ...
                                BaseSecu3Packet.parse(
                                    line,
                                    packetBuffer.sliceArray(IntRange(0, idx - 1))
                                )?.let { packet ->
                                    runBlocking {
