...
data class StarterParamPacket(
    ...
    var fldClr_start: Int = 0

) : BaseOutputPacket(){

    var floodClearStart : Boolean
        get() = fldClr_start.getBitValue(0) > 0
        set(value) {fldClr_start = fldClr_start.setBitValue(value, 1)}


    companion object {

        internal const val DESCRIPTOR = 'o'
        internal const val SIZE_PACKET = 16

        fun parse(data: String) = StarterParamPacket().apply {
            starterOff = data.get2Bytes(2)
            smapAbandon = data.get2Bytes(4)
            crankToRunTime = data.get2Bytes(6).toFloat() / 100.0f
            injAftstrStroke = data[8].toInt() * 4
            injPrimeCold = data.get2Bytes(9).toFloat() * 32 / 10000.0f
            injPrimeHot = data.get2Bytes(11).toFloat() * 32 / 10000.0f
