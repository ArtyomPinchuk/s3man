...
data class FunSetParamPacket(
    ...
    override fun pack(): String {
        ...
        data += mapCurveOffset.times(UBAT_MULTIPLIER).toInt().write2Bytes()
        data += mapCurveGradient.div(100).times(2048).toInt().write2Bytes()
        data += map2CurveOffset.times(UBAT_MULTIPLIER).toInt().write2Bytes()
        data += map2CurveGradient.div(100).times(2048).toInt().write2Bytes()
        data += tpsCurveOffset.times(UBAT_MULTIPLIER).toInt().write2Bytes()
...
//        data += funcFlags.toChar()
    ...
    companion object {
        ...
        fun parse(data: String) = FunSetParamPacket().apply {
            ...
            mapCurveOffset = data.get2Bytes(8).toFloat() / UBAT_MULTIPLIER
            mapCurveGradient = data.get2Bytes(10).toFloat() / 2048 * 100
            map2CurveOffset = data.get2Bytes(12).toFloat() / UBAT_MULTIPLIER
            map2CurveGradient = data.get2Bytes(14).toFloat() / 2048 * 100
            tpsCurveOffset = data.get2Bytes(16).toFloat() / UBAT_MULTIPLIER
