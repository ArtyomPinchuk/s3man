...
data class IdlingParamPacket(
    ...
    var idlRegPp: Float = 0f,
    var idlRegIp: Float = 0f,
    var idlRegPm: Float = 0f,
    var idlRegIm: Float = 0f,
    ...
    companion object {
        ...
        fun parse(data: String) = IdlingParamPacket().apply {
            ...
            idlRegPp = data.get2Bytes(19).toFloat() / 256
            idlRegIp = data.get2Bytes(21).toFloat() / 256
            idlRegPm = data.get2Bytes(23).toFloat() / 256
            idlRegIm = data.get2Bytes(25).toFloat() / 256
            coefThrd1 = data[27].toFloat().div(128).plus(1.0f)
            coefThrd2 = data[28].toFloat().div(128).plus(1.0f)
            integratorRpmLim = data[29].toInt().times(10)
            mapValue = data.get2Bytes(30).toFloat() / MAP_MULTIPLIER
            iacMinPos = data[32].toFloat() / 2
            iacMaxPos = data[33].toFloat() / 2
    ...
    override fun pack(): String {
        ...
        data += idlRegPp.times(256).toInt().toChar()
        data += idlRegIp.times(256).toInt().toChar()
        data += idlRegPm.times(256).toInt().toChar()
        data += idlRegIm.times(256).toInt().toChar()
